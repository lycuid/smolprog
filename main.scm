#!/usr/bin/env -S guile -e "(smolprog)" -l
!#

(define-module (smolprog))

(add-to-load-path (string-append (dirname (current-filename)) "/include"))
(use-modules ((ice-9 atomic))
             ((ice-9 threads))
             ((srfi srfi-43)  #:select (vector-map))
             ((srfi srfi-1)   #:select (fold-right))
             ((srfi srfi-18)  #:select (time->seconds seconds->time current-time
                                       thread-sleep!))
             (utils))
...
(define procs
  (vector (λ (i) (interval-runner  net:interval net:runner i))
          ...
          (λ (i) (fifo-runner      vol:fifo-path vol:default vol:fmt i))
...
(define fifo-runner
  (λ (fifo-path default fmt index)
     (begin
       (with-mutex mutex
                   (vector-set! output index (format #f fmt default))
                   (signal-condition-variable condvar))
       (while-running
         (let ([new (first-line-of fifo-path)])
           (with-mutex mutex
